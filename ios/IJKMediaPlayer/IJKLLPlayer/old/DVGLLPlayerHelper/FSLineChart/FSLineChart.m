...
- (void)strokeChartFor:(NSArray*)layerData
{
    CGFloat minBound = [self minVerticalBound];
    CGFloat maxBound = [self maxVerticalBound];
    ...
    UIBezierPath *noPath = [self getLinePath:0 withSmoothing:_bezierSmoothing close:NO in:layerData];
    UIBezierPath *path = [self getLinePath:scale withSmoothing:_bezierSmoothing close:NO in:layerData];
    
    UIBezierPath *noFill = [self getLinePath:0 withSmoothing:_bezierSmoothing close:YES in:layerData];
    UIBezierPath *fill = [self getLinePath:scale withSmoothing:_bezierSmoothing close:YES in:layerData];
...
- (void)strokeDataPointsFor:(NSArray*)layerData
{
    CGFloat minBound = [self minVerticalBound];
    CGFloat maxBound = [self maxVerticalBound];
    CGFloat scale = _axisHeight / (maxBound - minBound);
    ...
    for(int i=0;i<layerData.count;i++) {
        CGPoint p = [self getPointForIndex:i withScale:scale in:layerData];
        p.y +=  minBound * scale;
...
- (CGPoint)getPointForIndex:(NSUInteger)idx withScale:(CGFloat)scale in:(NSArray*)layerData
{
    if(idx >= layerData.count)
        return CGPointZero;
    
    // Compute the point position in the view from the data with a set scale value
    NSNumber* number = layerData[idx];
    
    if(layerData.count < 2) {
...
- (UIBezierPath*)getLinePath:(float)scale withSmoothing:(BOOL)smoothed close:(BOOL)closed in:(NSArray*)layerData
{
    ...
    if(smoothed) {
        for(int i=0;i<layerData.count - 1;i++) {
            ...
            // First control point
            nextPoint = [self getPointForIndex:i + 1 withScale:scale in:layerData];
            previousPoint = [self getPointForIndex:i - 1 withScale:scale in:layerData];
            m = CGPointZero;
            ...
            nextPoint = [self getPointForIndex:i + 2 withScale:scale in:layerData];
            previousPoint = [self getPointForIndex:i withScale:scale in:layerData];
            p = [self getPointForIndex:i + 1 withScale:scale in:layerData];
            m = CGPointZero;
        ...
        for(int i=0;i<_data.count;i++) {
            if(i > 0) {
                [path addLineToPoint:[self getPointForIndex:i withScale:scale in:layerData]];
            } else {
                [path moveToPoint:[self getPointForIndex:i withScale:scale in:layerData]];
            }
        }
    }
    ...
    if(closed) {
        ...
        [path addLineToPoint:[self getPointForIndex:_data.count - 1 withScale:scale in:layerData]];
        [path addLineToPoint:[self getPointForIndex:_data.count - 1 withScale:0 in:layerData]];
        [path addLineToPoint:[self getPointForIndex:0 withScale:0 in:layerData]];
        [path addLineToPoint:[self getPointForIndex:0 withScale:scale in:layerData]];
    }
